...
app.post('/maquiagem', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
        ...
        const resultado = await conn.query("INSERT INTO maquiagem (id,nome_produto,quantidade_produto,validade) VALUES (?,?,?,?)",[id,nome_produto,quantidade_produto,validade])
        const [dados, camposTabela] = resultado
        reply.status(200).send(dados)
    }
    catch (erro: any) {
...
app.get('/esmalte', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
        ...
        const resultado = await conn.query("SELECT * FROM esmalte")
        const [dados, camposTabela] = resultado
        reply.status(200).send(dados)
    }
